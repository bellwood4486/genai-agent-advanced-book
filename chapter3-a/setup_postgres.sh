...
while [ $COUNTER -lt $TIMEOUT ]; do
    if docker exec postgres-genai-ch3a pg_isready -U testuser -d testdb &>/dev/null; then
...
if docker exec postgres-genai-ch3a psql -U testuser -d testdb -c "SELECT COUNT(*) FROM employees;" &>/dev/null; then
    employee_count=$(docker exec postgres-genai-ch3a psql -U testuser -d testdb -t -c "SELECT COUNT(*) FROM employees;" | xargs)
...
if docker exec postgres-genai-ch3a psql -U testuser -d testdb -c "SELECT version();" &>/dev/null; then
...
echo
print_info "=== セットアップ情報 ==="
print_info "コンテナ名: postgres-genai-ch3a"
...
print_info "  docker exec -it postgres-genai-ch3a psql -U testuser -d testdb"
echo
print_info "employeesテーブルの確認:"
print_info "  docker exec postgres-genai-ch3a psql -U testuser -d testdb -c \"SELECT * FROM employees;\""
echo
print_info "コンテナの停止:"
...
print_info "1. .envファイルにAnthropic API Keyを設定してください"
